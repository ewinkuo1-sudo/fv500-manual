---
description: '常用螢光染劑與 FV500 雷射波長、偵測通道的對照，以及串色與光漂白對策'
---
...
下表通道為**雙標配置下的典型分配**。實際通道由偵測分光鏡（Mirror / DM570 / DM630）的設定決定，並非染劑固有屬性。單染時設定 **Mirror** 模式，所有螢光統一進 CH1。
...
讓兩種雷射**不同時開啟**，從物理上消除激發串色。分 **Line Sequential**（逐線切換，快，適合活細胞）與 **Frame Sequential**（逐張切換，慢，適合固定樣品或嚴重串色）兩種模式。

模式比較表與設定步驟詳見[第6章 進階掃描模式](/advanced-scan#sequential-scan)。
