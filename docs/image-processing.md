...
## 7.1 Display 面板與顯示功能（不改變資料）

顯示功能只改變「怎麼看」，**不改變存下來的像素值**。

| 功能 | 說明 | 用途 |
| --- | --- | --- |
| **LUT / 偽色** | 畫面左下工具列 `<LUT>` 按鈕 → [Color Tool]，把灰階映射成顏色（Green、Red 等） | 多通道各給一色，或凸顯強度差異 |
| **Hi-Lo** | [Color Tool] 的 Standard Color LUTs 之一：飽和顯示紅色、零值顯示藍色 | 調參數時檢查過曝與截波 |
